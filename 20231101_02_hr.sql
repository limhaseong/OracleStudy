...
-- 쉽게 하려면 이렇게 하면 되지만 이렇게 하지말라고 했음.

SELECT DEPARTMENT_ID
FROM DEPARTMENTS
WHERE DEPARTMENT_NAME = 'IT';

SELECT FIRST_NAME, LAST_NAME, SALARY, DEPARTMENT_ID , SALARY *1.1 "10%인상된급여"
FROM EMPLOYEES
...
SELECT FIRST_NAME, LAST_NAME, SALARY, DEPARTMENT_ID , SALARY, EXTRACT(YEAR FROM HIRE_DATE)
FROM EMPLOYEES
...
WHERE JOB_ID = (SELECT JOB_ID
                ...
                WHERE JOB_TITLE = 'Sales Manager')
AND EXTRACT(YEAR FROM HIRE_DATE) < 2006;
-- TO_NUMBER(TO_CHAR(HIRE_DATE, 'YYYY'))도 가능(숫자형태로 가져와야됨)
...
SELECT  CASE WHEN DEPARTMENT_ID = 90 THEN SALARY*1.1
            WHEN DEPARTMENT_ID = 100 THEN SALARY*1.15
            WHEN DEPARTMENT_ID = 110 THEN SALARY*1.2
            ELSE SALARY
            END "인상후"
FROM EMPLOYEES
...
(
    SELECT E.SALARY
            , CASE WHEN D.DEPARTMENT_NAME = 'Finance' THEN SALARY *1.1
                    WHEN D.DEPARTMENT_NAME = 'Executive' THEN SALARY *1.15
                    WHEN D.DEPARTMENT_NAME = 'Accounting' THEN SALARY *1.2
                    ELSE -1
                END "SAPLUS"
...
WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
                        FROM DEPARTMENTS
                        WHERE DEPARTMENT_NAME IN ('Finance', 'Executive', 'Accounting'));       
--※ WHERE절을 빼면 107개가 업데이트 됨(WHERE절이 있으면 11개 행이 업데이트 됨)
--   둘이 같은 결과가 나오지만 메모리에 올리는 갯수가 다름. 따라서 결과는 나와도 WHERE문 쓰기
...
-- 1. FROM을 생략해도 되지만 꼭 쓰자
-- 2. DELETE를 쓰기 전에 꼭 SELETE로 확인하고 SELECT를 DELETE를 지우자
SELECT *
FROM EMPLOYEES
