...
--==> DBA로 시작하는 Oracle Data Dictionary View는
...
SEGMENT SPACE MANAGEMENT AUTO;        -- 세그먼트 공간 관리로 오라클 서버가 자동으로 관리
...
--○ 오라클 사용자 계정 생성
CREATE USER lhs IDENTIFIED BY java006$     -- 여기까지 쓰고 ;해도 생성은 됨.
DEFAULT TABLESPACE TBS_EDUA;
--==>User LHS이(가) 생성되었습니다.
--> lhs 라는 사용자 계정을 생성하겠다(만들겠다, 만들어줘)
--  이 사용자 계정의 패스워드는 java006$ 로 하겠다.
--  이 계정을 통해 생성하는 오라클 세그먼트는
--  기본적으로 TBS_EDUA라는 테이블스페이스에 생성할 수 있도록 설정하겠다.
...
GRANT CREATE SESSION TO lhs; --대문자도 가능
--==> Grant을(를) 성공했습니다.

--○ 각자 생성한 오라클 사용자 계정의 시스템 관련 권한 조회
SELECT *
FROM DBA_SYS_PRIVS;
--==>LHS	CREATE SESSION	NO

-----------------------------------------------------------------------------

--○ 각자 생성한 오라클 사용자 계정에
--   테이블 생성이 가능하도록 CREATE TABLE 권한 부여
GRANT CREATE TABLE TO LHS;
--==> Grant을(를) 성공했습니다.
