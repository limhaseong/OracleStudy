...
BEGIN
    ...
    VALUES(('PF' || LPAD(TO_CHAR(PROFESSORS_SEQ.NEXTVAL), 3, '0')), V_PF_ID, SUBSTR(V_PF_SSN, 8, 7), V_PF_NAME, V_PF_SSN);
...
BEGIN
    ...
    FROM OPEN_SUBJECTS
    WHERE OCRS_CODE ='OC001'-- OCRS_CODE_TEMP
      AND OSUB_CODE ='SB007'-- V_OSUB_CODE;
...
BEGIN    
    ...
    SELECT COUNT(*) INTO FLAG_PW
    FROM STUDENTS
    WHERE ST_PW = V_ST_PW;

    IF FLAG_PW = 1
        THEN RAISE_APPLICATION_ERROR(-20051, '이미 등록된 주민번호입니다.' );
        ROLLBACK;
    END IF;
...
IS
 VV_OCRS_CODE      COURSE_REGISTRATIONS.ST_CODE%TYPE;   
 VV_START_DATE     OPEN_COURSES.START_DATE%TYPE;
 VV_END_DATE       OPEN_COURSES.END_DATE%TYPE;
BEGIN

    SELECT START_DATE  INTO VV_START_DATE
    FROM OPEN_COURSES
    WHERE OCRS_CODE=V_OCRS_CODE;    

    IF(VV_START_DATE<SYSDATE)
     THEN RAISE_APPLICATION_ERROR(-20004,'수강신청이 끝난 과목입니다.');
        ROLLBACK;
    END IF;
    
    /*
    SELECT  OCRS_CODE  INTO VV_OCRS_CODE
    FROM COURSE_REGISTRATIONS
    WHERE ST_CODE = V_ST_CODE;
    
    IF( V_OCRS_CODE = VV_OCRS_CODE )
        THEN  RAISE_APPLICATION_ERROR(-20005, '똑같은 과정을 신청했습니다.');
        ROLLBACK;
    END IF;
    
    
    SELECT O.END_DATE  INTO VV_END_DATE
    FROM OPEN_COURSES O
    JOIN COURSE_REGISTRATIONS C 
    ON O.OCRS_CODE = C.OCRS_CODE
    WHERE C.OCRS_CODE=V_OCRS_CODE;
    
    IF( VV_END_DATE>VV_START_DATE )
        THEN  RAISE_APPLICATION_ERROR(-20006, '신청한 과목이 이미 신청한 과목과 기간이 겹쳐요~');
        ROLLBACK;
    END IF;
    */
    ...
    VALUES(('RG' || LPAD(TO_CHAR(COURSE_REGISTRAIONS_SEQ.NEXTVAL), 3, '0')), V_ST_CODE, V_OCRS_CODE, SYSDATE);
END;
...
BEGIN        
   ...
   WHERE OS.SUB_CODE = V_SUB_CODE
    ...
    IF (HMM=0)
     THEN RAISE_APPLICATION_ERROR(-20007, '조건에 맞는 학생이없어요');
       ROLLBACK;
   END IF;
  ...
  SELECT  OS.OSUB_CODE INTO  V_OSUB_CODE
   ...
   WHERE REG_CODE = V_REG_CODE;
    ...
    IF(HA>0)
        THEN  RAISE_APPLICATION_ERROR(-20005, '이미 성적입력을 했어요.');
      ROLLBACK;       
    END IF;
    ...
    IF((V_ATTENDANCE_SCORE>V_A_SCORE) OR (V_WRITTEN_SCORE>V_W_SCORE) OR (V_PRACTICAL_SCORE>V_P_SCORE))
        THEN  RAISE_APPLICATION_ERROR(-20005, '배정된 최대 배점보다 점수가 커요~!');
      ROLLBACK;       
    END IF;
    
     IF(V_SUB_END >SYSDATE)
        THEN 
            RAISE_APPLICATION_ERROR(-20006, '아직 과목기간이 안끝났어요!');
       ROLLBACK; 
    END IF;
