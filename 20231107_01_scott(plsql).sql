...
*/
--20231106_01_scott(plsql).sql에 방법 1
...
BEGIN
    ...
    --    (재고수량 + 이전출고수량 < 현재출고수량 인 상황이라면... 사용자정의예외발생
...
IS
    V_상품코드 TBL_상품.상품코드%TYPE;
    V_재고수량 TBL_상품.재고수량%TYPE;
    V_이전수량 TBL_입고.입고수량%TYPE;
    USER_DEFINE_ERROR EXCEPTION;
    
BEGIN
    SELECT 상품코드, 입고수량 INTO V_상품코드, V_이전수량
    FROM TBL_입고
    WHERE 입고번호 = V_입고번호;
...
--    그러나 SQL문을 실행한 결과문(RESULT SET)이
--    여러 행(ROW)으로 구성된 경우
...
BEGIN
    ...
    -- 커서 오픈 시 (와아~~)쏟아져나오는 데이터를 처리(반복문을 활용하여 처리)
    LOOP
...
CREATE [OR REPLACE] TRIGGER 트리거명
    ...
    [FOR EACH ROW [WHEN TRIGGER 조건]]             -- FOR EACH ROW -> STATEMENT 사용시 생략
[DECLARE]
    -- 선언 구문;
BEGIN
    -- 실행 구문;
...
--※ DML 작업에 대한 이벤트 기록
-- EX) 떠든 사람 이름 적기 -> 떠든 이후에 사람 적기 때문에 AFTRT, 사람만 적기 때문에 STATEMENT
...
BEGIN
    IF (작업시간이 오전 9시 이전이거나... 오후 6시 이후라면)
        THEN 작업을 수행하지 못하도록 처리하겠다.(예외를 발생시키도록 처리하겠다.)
    END IF;
END;
...
BEGIN
    DELETE
    FROM TBL_TEST3
    WHERE CODE = :OLD.CODE;            --   :OLD   현재   :NEW
END;
